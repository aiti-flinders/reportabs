...
- \fancypagestyle{common}{\fancyhf{}\setlength{\headheight}{15pt}\fancyfoot[C]{\vspace{10mm}\thepage}\fancyfoot[L]{{\transparent{0.75}\includegraphics[height = 20mm]{img/aiti_logo.png}}}\fancyfoot[R]{{\transparent{0.75}\includegraphics[height = 20mm]{`r paste0("img/", strayr::clean_states(params$state),"_map.png")`}}}\fancyhead[L]{\textbf{Labour Market Brief}}\fancyhead[C]{}\fancyhead[R]{`r params$state`}\renewcommand{\footrulewidth}{1.0pt}}
...
ggplot(plot_data, aes(x = date, y = index, colour = indicator, linetype = indicator)) +
  ...
  labs(
    ...
    title = paste0("FULL AND PART-TIME EMPLOYMENT: ", strayr::clean_states(params$state) %>% str_to_upper),
...
ggplot(plot_data, aes(x = date, y = value, colour = gender, linetype = gender)) +
  ...
  labs(
    x=NULL,
    y=NULL,
    title = paste0("UNEMPLOYMENT RATES: MALE AND FEMALE: ", strayr::clean_states(params$state) %>% str_to_upper),
